...
public class EventListener implements Listener {
	...
	public void OnEntityExplodeEvent(EntityExplodeEvent event) {
		if(plugin.getConfig().getBoolean("disable-all-explotions")) {
			//event.setCancelled(true);
			event.blockList().clear();
			return;
		}
		EntityType ent = event.getEntityType();
		if(plugin.getConfig().getBoolean("disable-creeper-explotions") && ent.equals(EntityType.CREEPER)) {
			event.blockList().clear();
			return;
		}
		if(plugin.getConfig().getBoolean("disable-dragon-explotions") && ent.equals(EntityType.ENDER_DRAGON)) {
			event.blockList().clear();
			return;
		}
		if(plugin.getConfig().getBoolean("disable-tnt-explotions") && (ent.equals(EntityType.MINECART_TNT) ||ent.equals(EntityType.PRIMED_TNT))) {
			event.blockList().clear();
			return;
		}
	}
	...
	public void onEDamage(EntityDamageEvent e) {
		...
		if(e.getEntity() instanceof Player) {
			...
			if(damagee.hasMetadata("isdown") && damagee.getMetadata("isdown").get(0).asBoolean()) {
				...
				try {
					//TODO
					if(teams.getTeam(damagee)!= null) {
						if(damagee.hasMetadata("team_name")) {
							board.makeScoreBoard(damagee.getMetadata("team_name").get(0).asString());
				...
				if((damagee.getHealth() - e.getFinalDamage()) <= .5) {
					...
					if(teams.getTeam(damagee)!= null) {
						if(damagee.hasMetadata("team_name")) {			        	board.makeScoreBoard(damagee.getMetadata("team_name").get(0).asString());
			        	board.setScoreboard(team.getMembersPlayerObjects(), damagee.getMetadata("team_name").get(0).asString(), damagee, (int) ((damagee.getHealth()-e.getFinalDamage())*-1));
						}
			        }
		...
		if(event.getEntityType().equals(EntityType.PLAYER)) {
			...
			try {
				if(teams.getTeam(player)!= null) {
					if(player.hasMetadata("team_name")) {
						MSTeam team = teams.getTeam(player);
						board.makeScoreBoard(player.getMetadata("team_name").get(0).asString());
